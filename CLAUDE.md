...
`bbob-jax` is a JAX implementation of the 24 BBOB (Black-Box Optimization Benchmark) noise-free functions, the 30 BBOB-noisy functions (f101–f130), the 25 CEC 2005 benchmark functions, the 29 CEC 2017 bound-constrained functions (F2 was officially withdrawn and is skipped; numbering keeps the hole), and the 15 CEC 2013 Large-Scale Global Optimization functions (F1–F15). The key value-add over the original C implementations is full JAX compatibility: automatic differentiation, JIT compilation, and vectorization via `vmap`.

Unlike the other four suites (whose instance parameters are seed-generated), **CEC 2013 LSGO is a fixed-instance suite**: its parameters are the official constants, vendored as `.npz` package data (`_src/cec2013lsgo_data/`, ported from MetaBox — see `THIRD_PARTY_NOTICES.md`). Each LSGO function is defined only at its native dimension (1000, or 905 for the overlapping F13/F14); the maker validates `ndim`, ignores `key`, and has no deterministic variant. See `CONTEXT.md` and `docs/adr/` conventions.
...
- `cec2013lsgo_registry` — CEC 2013 LSGO factory (names `cec2013lsgo_f1`…`cec2013lsgo_f15`). Fixed-instance suite: **no `_original` variant**; each maker validates `ndim` against the native dimension and ignores `key`
- `cec2013lsgo_function_characteristics` — CEC 2013 LSGO metadata dict (one-hot `separable`/`partially_separable`/`overlapping`/`non_separable`, plus `rotated`)
- `cec2013lsgo_bounds` — CEC 2013 LSGO per-function bounds (±100, ±5 for Rastrigin-based, ±32 for Ackley-based)
...
└── _src/
    ...
    ├── cec2013lsgo.py   # All 15 CEC 2013 LSGO functions + fixed-instance loader
    ├── cec2013lsgo_data/ # Vendored LSGO reference constants (*.npz) + PROVENANCE.md
    ├── cec2013lsgo_tags.py  # cec2013lsgo_function_characteristics, derived from spec.py
...
Tests live in `tests/` (`test_example.py` for BBOB, `test_bbob_noisy.py` for BBOB-noisy, `test_cec2005.py` for CEC 2005, `test_cec2017.py` for CEC 2017, `test_cec2013lsgo.py` for CEC 2013 LSGO, plus `test_bounds.py`, `test_composition.py`, `test_problem.py`, `test_consistency.py`, `test_gallagher_equivalence.py`). The LSGO test loads reference regression pins from `tests/data/cec2013lsgo_golden.npz` (generated by `scripts/crosscheck_cec2013lsgo.py`) and runs in float64. They are parametrized over all functions × multiple dimensions × both registries. Each test validates:
