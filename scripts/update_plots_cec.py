...
from bbob_jax import cec2005_bounds, cec2005_registry_original
...
def update_plots() -> None:
    ...
    for (name, func), bounds in zip(
        cec2005_registry_original.items(), cec2005_bounds.values(), strict=True
    ):
    ...
    for i, ((name, func), bounds) in enumerate(
        zip(
            cec2005_registry_original.items(),
            cec2005_bounds.values(),
            strict=True,
        )
    ):
        ax = axes[i]
    ...
    plt.tight_layout()
    plt.savefig("img/cec2005_functions_overview_2d.png", dpi=300)
    plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig("img/cec2005_functions_overview_3d.png", dpi=300)
    plt.close(fig)
